...
export default function FlashcardApp() {
  ...
  const [testLimit, setTestLimit] = useState(10); // '10' or 'all'
  const [testCategory, setTestCategory] = useState('cards'); // 'cards', 'paraphrases'

  // Reading Practice mode - NEW
  ...
  const [showReadingImport, setShowReadingImport] = useState(false);
  ...
  // Test mode functions
  const startTest = (type, category = 'cards') => {
    let sourceData = [];
    ...
    if (category === 'cards') {
      ...
      if (selectedUnits.length > 0) {
        sourceData = cards.filter(card => selectedUnits.includes(card.unit || 'General'));
      } else {
        sourceData = cards;
      }
    ...
    } else if (category === 'paraphrases') {
      if (paraphrases.length === 0) {
        alert('No paraphrases available for testing!');
        return;
      }
      sourceData = paraphrases.map(p => ({
        id: p.id,
        front: p.original,
        back: p.variations[0], // Primary answer for display
        variations: p.variations, // All valid answers
        type: 'paraphrase'
      }));
    }
  ...
  const isAnswerCorrect = (answer, card) => {
    const cleanAnswer = answer.toLowerCase().trim();

    if (card.type === 'paraphrase' && card.variations) {
      return card.variations.some(v => v.toLowerCase().trim() === cleanAnswer);
    }
  ...
  const answerTestQuestion = (answer) => {
    ...
    const newAnswers = {
      ...
      [currentCard.id]: {
        ...
        isCorrect: isAnswerCorrect(answer, currentCard)
      }
    };
  ...
  const highlightText = () => {
    ...
    setHighlightedWords([...highlightedWords, newHighlight]);
    setShowContextMenu(false);
  ...
  // Add highlighted word to flashcards
  const addHighlightToFlashcard = async (highlight) => {
    const definition = await lookupWord(highlight.text);
    if (definition) {
      const newCard = {
        front: definition.word,
        back: definition.definition,
        example: definition.example || '',
        translation: '',
        unit: 'Reading Practice',
        level: 'Beginner',
      };

      try {
        const saved = await addSharedCard(newCard);
        const now = Date.now();
  ...
  // Import reading text
  const importReadingText = () => {
    if (!readingText.trim()) {
      alert('Please paste some text first!');
      return;
    }

    setShowReadingImport(false);
    // Text is already in state
  };
              ...
              <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-lg">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {getUniqueUnits().map(unit => (
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg border-2 transition-all ${selectedUnits.includes(unit)
                ...
                <div className="mb-6 pb-6 border-b border-slate-200">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg border-2 transition-all ${testLimit === 10
                        ? 'border-indigo-600 bg-indigo-50 text-indigo-700 font-medium'
                        : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg border-2 transition-all ${testLimit === 'all'
                        ? 'border-indigo-600 bg-indigo-50 text-indigo-700 font-medium'
                        : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'
                        }`}
                    >
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-6">
                  <div className="space-y-4">
                    ...
                    <button
                      onClick={() => startTest('written', 'cards')}
                    ...
                    <button
                      onClick={() => startTest('multiple-choice', 'cards')}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-slate-900 group-hover:text-green-600">
                            Multiple Choice
                          </div>
                          <div className="text-xs text-slate-500">
                  ...
                  <div className="space-y-4">
                    ...
                    <button
                      onClick={() => startTest('written', 'paraphrases')}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-slate-900 group-hover:text-purple-600">
                            Paraphrase Recall
                          </div>
                          <div className="text-xs text-slate-500">
                            Type any valid variation
                          </div>
                        </div>
                      </div>
                    </button>
                    <div className="bg-slate-50 p-3 rounded-lg text-xs text-slate-500 italic">
                      More paraphrase test modes coming soon!
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-lg">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="text-sm font-medium text-indigo-600 mono">
                      {testCategory === 'paraphrases' ? 'Paraphrase Test' : (testType === 'written' ? 'Written Test' : 'Multiple Choice')}
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-lg slide-in">
                          ...
                          <div className="flex-1">
                            ...
                            {answer.isCorrect && (
                              <div className="text-sm text-green-700 mono">
                                ...
                                {testCategory === 'paraphrases' && (
                                  <div className="text-xs text-slate-500 mt-1">
                                    (Main variation: {answer.correct})
                                  </div>
                                )}
                              </div>
                            )}
                ...
                <div className="flex gap-4">
                  <Button
                    onClick={() => {
                      resetTest();
                      startTest(testType, testCategory);
                    }}
          ...
          <div className="slide-in">
            {readingText ? (
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                  <div className="flex justify-between items-center gap-4 flex-wrap">
                    <div className="flex-1">
                      <h2 className="text-2xl font-bold text-slate-900">{readingTitle || 'Reading Passage'}</h2>
                      <p className="text-sm text-slate-600">Click any word to see definition • Select text to highlight</p>
                    </div>
                    <div className="flex gap-2">
                      <Button
                        onClick={() => {
                      ...
                      <Button
                        onClick={() => setShowReadingImport(true)}
                        size="sm"
                      >
                        <Upload className="w-4 h-4 mr-2" />
                        New Text
                ...
                <div className="bg-white rounded-xl p-8 border border-slate-200 shadow-sm">
                  ...
                  >
                    {readingText.split(/\s+/).map((word, index) => {
                      const isHighlighted = highlightedWords.some(h =>
                        h.text.toLowerCase().includes(word.toLowerCase().replace(/[.,!?;:]/g, ''))
                      );
                      ...
                      return (
                        <span
                          ...
                          className={`cursor-pointer hover:bg-indigo-50 px-1 rounded transition-colors ${isHighlighted ? 'bg-yellow-200' : ''
                            }`}
                          style={{
                ...
                {highlightedWords.length > 0 && (
                  <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
                    ...
                    <div className="space-y-2">
                      {highlightedWords.map((highlight) => (
                        <div
                          key={highlight.id}
                          className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200"
                        >
                          <div className="flex-1">
                            <span className="font-medium text-slate-900">{highlight.text}</span>
                          </div>
                          <div className="flex gap-2">
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => addHighlightToFlashcard(highlight)}
                            >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() => {
                                setHighlightedWords(highlightedWords.filter(h => h.id !== highlight.id));
                              }}
                ...
                {selectedWord && (
                  <div
                    className="fixed z-50 bg-white rounded-lg shadow-2xl border-2 border-green-500 p-4 max-w-sm"
                    style={{
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="text-xl font-bold text-slate-900">{selectedWord.word}</h4>
                        {selectedWord.phonetic && (
                          <p className="text-sm text-slate-500 mono">{selectedWord.phonetic}</p>
                        )}
                      </div>
                      <Button
                        size="sm"
                ...
                {showContextMenu && (
                  <div
                    className="fixed z-50 bg-white rounded-lg shadow-xl border border-slate-200 p-2 flex gap-2"
                    style={{
              ...
              <div className="bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-lg">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-2">Reading Practice</h3>
                <p className="text-slate-600 mb-6">
                  Import a reading passage to practice vocabulary with interactive definitions and highlighting.
                </p>
                <Button onClick={() => setShowReadingImport(true)} className="hover-lift">
                  <Upload className="w-4 h-4 mr-2" />
                  Import Reading Text
                </Button>
              </div>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 slide-in">
            <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-slate-900">Import Reading Text</h2>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => setShowReadingImport(false)}
                >
                  <X className="w-5 h-5" />
                </Button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g., The Kākāpō, IELTS Reading Passage 1..."
                    className="text-lg"
                  />
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-slate-500 mt-2">
                    💡 Tip: Copy from IELTS practice, news articles, or any English text
                  </p>
                </div>

                <div className="flex gap-3">
                  <Button
                    onClick={importReadingText}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`p-3 rounded-lg border-2 text-left transition-all ${importFormat === 'csv'
                    ...
                    <button
                      ...
                      className={`p-3 rounded-lg border-2 text-left transition-all ${importFormat === 'tsv'
                        ? 'border-indigo-600 bg-indigo-50'
                        : 'border-slate-200 hover:border-slate-300'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`p-3 rounded-lg border-2 text-left transition-all ${importFormat === 'quizlet'
                        ? 'border-indigo-600 bg-indigo-50'
                        : 'border-slate-200 hover:border-slate-300'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`p-3 rounded-lg border-2 text-left transition-all ${importFormat === 'json'
                        ? 'border-indigo-600 bg-indigo-50'
                        : 'border-slate-200 hover:border-slate-300'
                        }`}
                    >
